...
document.addEventListener('DOMContentLoaded', function() {
    ...
    chrome.storage.sync.get(['p_key', 'year', 'semester', 'exam', 'department', 'degree', 'course_number'], function(result) {
        if (result.p_key) {
...
document.getElementById("open_graph").addEventListener("click", function() {
    Promise.all([
        ...
        new Promise((resolve) => chrome.storage.sync.get("exam", resolve)),
...
document.getElementById("popup_form").addEventListener("submit", function() {
    ...
    let examInput = document.querySelector('input[name="exam"]:checked');
    let exam = examInput ? examInput.value : null;
    ...
    if(exam) chrome.storage.sync.set({exam: exam});
    ...
    if(!p_key && !year && !semester && !exam && !department && !degree && !course_number) {
